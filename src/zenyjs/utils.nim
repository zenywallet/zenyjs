...
when defined(js):
  ...
  proc hmac_sha1*(key, data: Array[byte]): Uint8Array =
    withStack:
  ...
  proc hmac_sha256*(key, data: Array[byte]): Uint8Array =
    withStack:
      var p = Module.stackAlloc(32)
      Utils.hmac_sha256(key.handle, data.handle, p)
      result = newUint8Array(Module.HEAPU8.buffer, p.to(cint), 32).slice().to(Uint8Array)

  proc hmac_sha512*(key, data: Array[byte]): Uint8Array =
    withStack:
  ...
  proc hmac_sha256*(key, data: openArray[byte]): array[32, byte] {.inline.} =
    hmac_sha256(cast[ptr UncheckedArray[byte]](unsafeAddr key[0]), key.len.uint32,
                cast[ptr UncheckedArray[byte]](unsafeAddr data[0]), data.len.uint32)

  proc hmac_sha512*(key, data: openArray[byte]): array[64, byte] {.inline.} =
    hmac_sha512(cast[ptr UncheckedArray[byte]](unsafeAddr key[0]), key.len.uint32,
              cast[ptr UncheckedArray[byte]](unsafeAddr data[0]), data.len.uint32)

  proc hmac_sha1*(key: string, data: openArray[byte]): array[20, byte] {.inline.} =
    hmac_sha1(cast[ptr UncheckedArray[byte]](unsafeAddr key[0]), key.len.uint32,
              cast[ptr UncheckedArray[byte]](unsafeAddr data[0]), data.len.uint32)

  proc hmac_sha256*(key: string, data: openArray[byte]): array[32, byte] {.inline.} =
    hmac_sha256(cast[ptr UncheckedArray[byte]](unsafeAddr key[0]), key.len.uint32,
                cast[ptr UncheckedArray[byte]](unsafeAddr data[0]), data.len.uint32)

  proc hmac_sha512*(key: string, data: openArray[byte]): array[64, byte] {.inline.} =
    hmac_sha512(cast[ptr UncheckedArray[byte]](unsafeAddr key[0]), key.len.uint32,
                cast[ptr UncheckedArray[byte]](unsafeAddr data[0]), data.len.uint32)
